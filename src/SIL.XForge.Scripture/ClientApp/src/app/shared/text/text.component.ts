...
export class TextComponent implements OnDestroy {
  ...
  @Input() syncText?: TextComponent;
  @Output() updated = new EventEmitter<TextUpdatedEvent>();
  ...
  private adjustSelection(): void {
    ...
    if (sel.index !== newSel.index || sel.length !== newSel.length) {
      this._editor.setSelection(newSel, 'silent');
    }
  }
  ...
  private syncScroll(): void {
    if (this.syncText == null || this.syncText._segment == null || !this._editor.hasFocus()) {
      return;
    }

    const thisRange = this._segment.range;
    const thisBounds = this._editor.selection.getBounds(thisRange.index);

    const otherRange = this.syncText._segment.range;
    const otherBounds = this.syncText._editor.selection.getBounds(otherRange.index);
    this.syncText._editor.scrollingContainer.scrollTop += otherBounds.top - thisBounds.top;
  }
}
